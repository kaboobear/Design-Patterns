...
class FlyweightEnemyFactory{
    ...
    getFlyweight(texture:string, color:string):FlyweightEnemy{
        ...
        this.flyweightsArray.forEach(elem => {
            if(elem.getColor() === color && elem.getTexture() === texture) flyweightElem = elem;
        })
...
const enemy1 = new EnemyItem("Goblin",5,factory.getFlyweight("Skin","Green"));
enemy1.showEnemy();
factory.showFlyweightsElements();

const enemy2 = new EnemyItem("Knight",10,factory.getFlyweight("Iron","Grey"));
enemy2.showEnemy();
factory.showFlyweightsElements();
